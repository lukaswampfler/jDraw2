...
public class StdSelectionTool implements DrawTool {
	...
	/**
	 * Is the given position on an already selected figure?
	 * @param x x-coordinate of point to check
	 * @param y y-coordinate of point to check
	 * @return whether the checked point is on an already selected figure.
	 */
	...
	public void mouseDown(int constrainedX, int constrainedY, MouseEvent e) {
		...
		// 1. check, whether mouse position is on an already
	...
	public void mouseDrag(int i, int j, java.awt.event.MouseEvent e) {
		...
		if (selMode) {
			...
			Set<Figure> sel = view.getModel().getFigures().filter(f -> contains(selRectangle, f.getBounds())).collect(Collectors.toSet());
